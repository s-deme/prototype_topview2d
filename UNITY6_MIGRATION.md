...
このプロジェクトは **Unity 6.3.22f1 (`6000.3.22f1`)** を対象に更新済みです。

## 更新内容

- `ProjectSettings/ProjectVersion.txt` を Unity 6.3.22f1 に更新
...
2. Editor に **Unity 6.3.22f1** を指定します。
3. インターネット接続した状態で開き、Package Manager の初回依存解決を完了させます。
4. `Assets/Scenes/Prototype.unity` を開き、Play を押します。

`Packages/packages-lock.json` は初回の正常な依存解決時に Unity が生成します。生成後は他の開発環境でも同じ lock file を共有してください。
